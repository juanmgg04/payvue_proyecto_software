...
func (r *repository) CreateIncome(ctx context.Context, i *income.Income) (*income.Income, error) {
	query := `
		INSERT INTO incomes (amount, source, date, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?)
	`

	result, err := r.db.ExecContext(ctx, query,
		i.Amount, i.Source, i.Date, i.CreatedAt, i.UpdatedAt,
	)
...
func (r *repository) GetAllIncomes(ctx context.Context) ([]income.Income, error) {
	query := `
		SELECT id, amount, source, date, created_at, updated_at
		FROM incomes
		ORDER BY date DESC
	`
	...
	for rows.Next() {
		var i income.Income
		err := rows.Scan(
			&i.ID, &i.Amount, &i.Source, &i.Date, &i.CreatedAt, &i.UpdatedAt,
		)
		if err != nil {
			return nil, income.ErrDatabaseError
		}
